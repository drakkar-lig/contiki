...
- USE_DIO: this macro is for the code that is specific to cases where we use collection tree.
  If set to false, then LRP works as LOADng

- To set the behavior of each node, there are a few macros:
  - LRP_IS_COORDINATOR: If set to 1, this node will periodically re-send DIO messages -- set to 0 for a leaf node that will not relay traffic.
  ...
  - LRP_RANDOM_WAIT makes the node wait a random time before sending DIO in response to a QRY or when retransmitting RREQ.
    Should be set to one and adjust the time (random between 0 to 0.5s).
  - LRP_RREQ_RETRIES,
  - LRP_RREQ_RATELIMIT: minimum interval (in ticks) between 2 consecutive RREQ.
  - LRP_R_HOLD_TIME.
